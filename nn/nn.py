...
class NeuralNetwork:
    ...
    def forward(self, X: ArrayLike) -> Tuple[ArrayLike, Dict[str, ArrayLike]]:
        ...
        """
        # Use the input matrix X as the initial activation (input for the first layer)
        A_curr = X
        # Create an empty dictionary to store intermediate Z (weighted input) and A (activation) values
        cache = {}

        # Loop over each layer in the neural network as defined by its architecture
        for i in range(len(self.arch)):
            # Retrieve the current layer's specifications from the network architecture
            l = self.arch[i]
            # Get the weights for the current layer from the parameters dictionary
            W_curr = self._param_dict["W" + str(i + 1)]
            # Get the biases for the current layer from the parameters dictionary
            b_curr = self._param_dict["b" + str(i + 1)]
            # Get the activation function (e.g., "relu", "sigmoid") for the current layer
            activation = l["activation"]

            # The activation of the previous layer is the input for the current layer
            A_prev = A_curr
            # Perform a single forward pass step for the current layer, calculating new activations
            A_curr, Z_curr = self._single_forward(W_curr, b_curr, A_prev, activation)

            # Store the weighted input (Z value) of the current layer in the cache for backpropagation
            cache['Z' + str(i + 1)] = Z_curr
            # Store the activation (A value) of the current layer in the cache, to be used as input for the next layer
